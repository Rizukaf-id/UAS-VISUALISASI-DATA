...
def relationship():
    ...
    with tab1:
        rating_duration = filtered_data[['Rating', 'Durasi(Menit)']]
        fig = px.scatter(rating_duration, x='Rating', y='Durasi(Menit)', title='Rating vs Duration')
        max_duration = rating_duration['Durasi(Menit)'].idxmax()
        min_duration = rating_duration['Durasi(Menit)'].idxmin()
        fig.add_annotation(x=rating_duration.loc[max_duration, 'Rating'], y=rating_duration.loc[max_duration, 'Durasi(Menit)'], text="Max")
        fig.add_annotation(x=rating_duration.loc[min_duration, 'Rating'], y=rating_duration.loc[min_duration, 'Durasi(Menit)'], text="Min")
        st.plotly_chart(fig)
    with tab2:
